...
/// internal to that assembly and so not reachable from here). This one models no filesystem, because these
/// tests are not about what the SSH adapter installs — they are about whether the descriptor the DigitalOcean
/// adapter produced survives the trip into it. The one thing it does record is every
/// <see cref="TargetDescriptor"/> connected with, since that is the evidence the SSH side reached the machine
/// the cloud side created.
/// </remarks>
