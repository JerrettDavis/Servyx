...
The `/games` page lists every game definition bundled with this Servyx installation — today that's four: Palworld, Minecraft (itzg), ARK: Survival Ascended, and Factorio, each loaded from its own file under `definitions/`. See [Supported games](../games.md) for what each one ships. Each card shows its declared trust tier, tags, whether mod installation is supported, and its **deployment profiles**: the distinct ways that game can be deployed and detected. Palworld's own definition declares two profiles — a Docker profile (`docker-thijsvanloef`, matching the `thijsvanloef/palworld-server-docker` image, where `.env` is authoritative and `PalWorldSettings.ini` is regenerated on every restart) and a bare-metal profile (`native-steamcmd`, where the INI is authoritative directly, with no regeneration step); the other three definitions each declare one Docker profile.

![The Games page listing a bundled definition and its deployment profiles](../images/games-catalogue.png)

**What you'll actually see depends on how Servyx is running.** The demonstration data set (`Servyx:DataSource=Mock`) shows Palworld's own two profiles, to illustrate the distinction. Running against a real Docker daemon, every loaded definition's card currently summarizes just its first Docker-kind deployment profile — so Palworld's second, bare-metal profile is not shown as a card entry today even though the underlying YAML declares it, and each card's trust/mod-support badges are shown at their safe defaults rather than reading the definition's own declared value. Neither is a reflection of what the schema supports — see [the schema reference](../schema.md) for what a full definition can declare.
...
For Docker, a container is proposed as a match for a game definition when **both** of the following hold, checked against every loaded definition, not just one:

- Its image repository matches a definition's expected repository (the tag or digest is ignored — `thijsvanloef/palworld-server-docker:latest` and `:v1.2` both match the same repository).
- It has a mount whose *container-side* destination matches the path that definition expects (for Palworld, the mount into `/palworld`; for Minecraft, `/data`).

If a container matches exactly one definition, that's the server's game. If it matches two or more with equal specificity, the server shows as ambiguous rather than guessing.
...
The Overview tab shows power controls (Start/Restart/Stop/Kill — locked unless this server has been granted write access, see [Lifecycle control](lifecycle-control.md)), status, network details, and storage/resource limits, all read from the live container.
