...
**Status: §11.8's item (1) is done and §11.8's item (2) is not.** `ProvisionedResource` now expresses unreachability, and *two* shape-M adapters exist on it: `AzureContainerInstanceProvisioner` (67 tests) and `AwsEcsFargateProvisioner` (133 tests). There is still no RCON control-plane path that consumes an unreachable resource, so a shape-M deployment remains something Servyx can create, price, sweep and destroy but cannot yet operate. See §11.10 for ACI and §11.11 for Fargate.
