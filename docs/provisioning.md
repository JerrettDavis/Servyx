...
| Type | Status in `src/` |
|---|---|
| `IPlanExecutor` | **Implemented and DI-registered**, in `Servyx.Config`/`PlanExecutor.cs`, registered at `ServyxCoreCompositionExtensions.cs:453-465`. `PreviewAsync` and `ApplyAsync` are real, and now have an operator-facing caller: `ChangePlanPanel.razor` on the settings tab previews a plan from recorded desired values and, behind a two-step confirmation, applies it. `RevertAsync` is also implemented — all-or-nothing preflight of pre-image availability, integrity, reversibility, and transport reachability, then a read-back-and-rehash verification of each restoring write — but no operator surface calls it: `ChangePlanPanel.razor` renders no revert affordance, so the engine capability exists without a caller. No REST API, MCP tool, or job runner calls any of the three methods — MCP support is a deliberate omission (a tool call cannot show a human a diff to approve) rather than a gap — so provisioning still has no path to route mutation through this funnel outside the settings-tab UI. |
| `ConfigChangePlan`, `PlannedAction`, `PlannedActionKind`, `Consequence`, `ConsequenceKind`, `ChangeReceipt`, `PlanStaleException`, `PlanApplyFidelityException`, `BlockedChange`, `PlanFeasibility`, `PlanDiagnostic` | **Exist as real types, and are now constructed/thrown in production code** by `PlanExecutor`. `BlockedChange` and `PlanFeasibility` in particular were previously listed below as not existing anywhere — they now do, with a narrower shape than this document's own `control-plane.md`-style sketch (`BlockedChange(SettingKey, SurfaceId, Reason, RemediationHint)`, no `MissingAlternatives`/`UnlockedAtTier`). |
| `ChangePlanRecord` / `ChangePlanActionRecord` | **Exist, with an applied EF migration, and are actively read/written** by `PlanExecutor` on every preview and apply. |
| `JobProgress` | **Exists** (`Servyx.Domain.Common`), used by backups. |
| `PlanStep`, `CapabilityFingerprint`, `JobStep`, `RestartImpact` | **Still do not exist anywhere in `src/` or `tests/`.** They are proposed here and in `control-plane.md`, and are named below as design vocabulary, not as code to call. (`BlockedChange` and `PlanFeasibility`, previously grouped with these, have since shipped — see the row above.) |

So provisioning is not "new step kinds inside an existing plan model" in the sense of extending running code. It is new step kinds and new capability requirements inside an existing *model sketch*, plus one new contract that produces those plans. The config-side executor the sketch has always assumed now exists (`PlanExecutor`, table above) — provisioning would extend it rather than build it — and it now has an operator-facing caller for configuration plans specifically (the settings tab), but that caller is settings-only and does not generalize to provisioning; the provisioning-specific vocabulary (`PlanStep`, `CapabilityFingerprint`, `JobStep`, `RestartImpact`) still has to be written before anything in this document compiles.
...
Extend `JobProgress` (which exists) and `IPlanExecutor` (which is now implemented for configuration plans — `PlanExecutor` in `Servyx.Config` — and now has an operator-facing caller for that one use case, the settings tab's `ChangePlanPanel`, but no provisioning-specific step kinds and no caller outside settings; extending it here means widening it, not building it from nothing); do not introduce a competing progress or execution abstraction. Add a database-backed queue and an in-process `JobRunner : BackgroundService` using row leases — no Hangfire, no Quartz, with single-instance documented as an assumption.
...
Drift is *designed*, not solved, for provisioning specifically — but the config-plan half of it is now real. `ConfigChangePlan.SurfaceHashes` captured at preview IS re-checked at apply by `PlanExecutor.ApplyAsync`: a pre-flight sweep re-reads every bound surface (not just the ones being written) and raises `PlanStaleException` before any write, plus a per-write TOCTOU check via `FileWriteOptions.ExpectedPreImageHash`/`TargetDriftException`. `ChangePlanStatus` and `ChangeReceipt` are both real and produced by that path. What provisioning would still need and does not have is `CapabilityFingerprint` — it does not exist at all — since a config plan's surface hashes have no equivalent "what can this connector do" fingerprint to stale against. Provisioning inherits the surface-hash half of the design and would still need to build the capability-fingerprint half.
...
The plan/preview/confirm flow is a `ChangePlan` variant rather than a second mechanism — and for configuration plans "the mechanism" is now a working flow (`PlanExecutor`, table in §1), not just record definitions and a database table; provisioning would still need to extend it with its own step kinds. The shape: `Previewed → Applied`, with `PlanFeasibility` of `FullyAchievable | PartiallyAchievable | Blocked`, and anything an adapter cannot do surfacing as a `BlockedChange` with a remediation hint. `PlanFeasibility`, `BlockedChange` and `RemediationHint` all exist and are used by `PlanExecutor.PreviewAsync` today — `PlanFeasibility`/`BlockedChange` only recently shipped; earlier drafts of this document (correctly, at the time) listed them as proposed-only.
