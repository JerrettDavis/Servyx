...
| Type | Status in `src/` |
|---|---|
| `IPlanExecutor` | **Implemented and DI-registered**, in `Servyx.Config`/`PlanExecutor.cs`, registered at `ServyxCoreCompositionExtensions.cs:453-465`. `PreviewAsync` and `ApplyAsync` are real; `RevertAsync` still throws `NotImplementedException`. What is still missing is a **caller** — no UI, REST API, MCP tool, or job runner invokes `PreviewAsync`/`ApplyAsync` anywhere in `src/`, so provisioning cannot yet route mutation through it in practice even though the funnel itself now works. |
