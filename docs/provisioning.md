...
| Type | Status in `src/` |
|---|---|
| `IPlanExecutor` | **Implemented and DI-registered**, in `Servyx.Config`/`PlanExecutor.cs`, registered at `ServyxCoreCompositionExtensions.cs:453-465`. `PreviewAsync` and `ApplyAsync` are real, and now have an operator-facing caller: `ChangePlanPanel.razor` on the settings tab previews a plan from recorded desired values and, behind a two-step confirmation, applies it. `RevertAsync` still throws `NotImplementedException`. No REST API, MCP tool, or job runner calls either method — MCP support is a deliberate omission (a tool call cannot show a human a diff to approve) rather than a gap — so provisioning still has no path to route mutation through this funnel outside the settings-tab UI. |
