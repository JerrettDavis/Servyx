...
**The rule, stated once and to be enforced everywhere below `IPlanExecutor`: no code beneath that boundary throws for a capability reason.** `IPlanExecutor` is now implemented (`PlanExecutor`, `Servyx.Config`), and the rule is honoured there: `PreviewAsync` turns an unwritable surface, missing capability, or unaddressable pointer into a `BlockedChange` rather than an exception — see `architecture.md`'s `IPlanExecutor` section. The one place the rule was already honoured before that, and still is underneath it, is surface resolution: `ISurfaceResolver.ResolveAsync` returns `SurfaceResolutionFailure` records for missing capabilities and container-scoping refusals rather than throwing. Capability is fully resolved during preview, before the user commits to anything, so the UI has the true picture — what will happen, what won't, why — *before* the "Apply" button is even enabled to be pressed. This inverts the failure mode that capability-unaware code naturally falls into, where a `NotSupportedException` surfaces from deep inside a file writer partway through applying a plan. By the time execution starts, capability is a known quantity; if it changes between preview and apply, that is handled explicitly as staleness (below, via `PlanStaleException`), not as an exception escaping a plan step. What this section's control-tier `CapabilityFingerprint`/`ControlCapabilitySet.Fingerprint` staleness check does *not* yet have is an apply-time enforcer of its own — `PlanExecutor.ApplyAsync`'s staleness sweep re-checks `ConfigChangePlan.SurfaceHashes`, not a capability fingerprint, because `CapabilityFingerprint` itself does not exist (see below).

`ConfigChangePlan` — which already exists as a declared record — is extended, not replaced.

**Two of the four types in the block below now exist in `src/`; two still do not.** `PlanFeasibility` and `BlockedChange` shipped as part of `PlanExecutor.PreviewAsync` and are real, constructed types today — though with a narrower shape than the sketch immediately below (the real `BlockedChange` is `(SettingKey, SurfaceId, Reason, RemediationHint)`, with no `MissingAlternatives` or `UnlockedAtTier`). `RestartImpact` and `PlanStep` are still proposed only; grepping for them across `src/` and `tests/` returns nothing. What else exists today: `ConfigChangePlan`, `PlannedAction`, `PlannedActionKind`, `Consequence`, `ConsequenceKind`, `ChangeReceipt`, `PlanStaleException`, `PlanApplyFidelityException`, `PlanDiagnostic`, `ControlCapability`, `ControlTier` and `RemediationHint`. Treat the block below as a target shape for the two types that remain unbuilt, not as an API to call — and note the real `PlanFeasibility`/`BlockedChange` shapes differ from what's sketched here:
...
`ConfigChangePlan` already carries `Feasibility` (a derived property) and a list of `BlockedChange` (via the `Blocked` init property), both real today — see `architecture.md`/`abstractions.md`. What it would still gain is a list of `PlanStep` and a `CapabilityFingerprint` captured at preview time; neither exists yet. Its base positional shape remains `(Id, Actions, Consequences, SurfaceHashes)`, with `Blocked` and `Diagnostics` as additional init-only properties defaulting to empty lists — chosen specifically so the four-parameter construction every existing caller/test uses keeps compiling.
