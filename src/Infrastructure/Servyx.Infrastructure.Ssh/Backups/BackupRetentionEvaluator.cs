...
/// directly, and <see cref="SshBackupProvider.PruneAsync"/>'s dry-run and live paths compute their answer
/// from the identical call rather than from two implementations that could disagree.
/// </para>
/// <para>
...
/// the three barriers described on <see cref="SshBackupProvider.PruneAsync"/>.
/// </para>
/// <para>
/// This duplicates <c>Servyx.Infrastructure.Docker.Backups.BackupRetentionEvaluator</c> — see the remarks on
/// <see cref="BackupGlob"/> for why, and note that this type, being pure policy over
/// <see cref="BackupArtifact"/>, is the strongest candidate of the four for promotion to
/// <c>Servyx.Domain.Backups</c> when that refactor is done on its own.
/// </para>
/// </remarks>
public static class BackupRetentionEvaluator
{
    ...
    /// any granularity keeps it, so a single nightly backup counts as that day's daily <em>and</em> that
