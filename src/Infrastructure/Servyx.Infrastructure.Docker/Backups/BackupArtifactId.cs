...
/// <summary>
/// Encodes and decodes the opaque <c>BackupArtifact.Id</c> strings this adapter hands out.
/// </summary>
/// <remarks>
/// <para>
/// <c>IBackupProvider.InspectAsync</c> and <c>PlanRestoreAsync</c> take a backup id and nothing else, so
/// the id has to carry enough to find the artifact again — which server it belongs to, and where it sits.
/// It is still treated as opaque by callers: resolution always goes back through a fresh listing and
/// matches on the whole id, so an id naming a file that has since been deleted, moved, or replaced fails
/// as "not found" rather than being trusted as a path to act on.
/// </para>
/// <para>
