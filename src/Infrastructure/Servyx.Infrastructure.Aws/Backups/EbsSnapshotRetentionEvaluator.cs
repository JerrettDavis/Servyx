...
/// <remarks>
/// <para>
/// <strong>The unit is a backup set, not a snapshot.</strong> One Servyx backup of an EC2 instance is every
/// EBS snapshot <c>CreateSnapshots</c> produced in one call, and half a set is not a backup of anything — a
/// root volume with no matching data volume restores a machine to a state it was never in. So retention
/// evaluates <em>sets</em>, and <see cref="EbsSnapshotBackupProvider.PruneAsync"/> deletes a set's snapshots
/// all together or not at all. This is the one place the EBS shape genuinely diverges from the DigitalOcean
/// evaluator, which had a single boot disk and therefore nothing to group.
/// </para>
/// <para>
/// Pure computation over a list of artifacts: it performs no I/O and deletes nothing, so it can be exercised
/// directly, and <see cref="EbsSnapshotBackupProvider.PruneAsync"/>'s dry-run and live paths compute their
...
/// second of the three barriers described on <see cref="EbsSnapshotBackupProvider.PruneAsync"/>.
/// </para>
/// <para>
/// Deliberately a separate implementation from the Docker, SSH and DigitalOcean adapters' evaluators, with the
/// same bucketing rule. Infrastructure projects reference <c>Servyx.Domain</c> and nothing else, so sharing it
/// would mean either a cross-adapter reference or promoting retention into the domain; a pinned test asserts
/// this one keeps the same set the others would.
/// </para>
/// </remarks>
public static class EbsSnapshotRetentionEvaluator
{
    /// <summary>
    ...
    /// distinct ISO-8601 weeks, all evaluated newest-first in UTC. An artifact survives if any granularity
    /// keeps it, so a single nightly capture counts as that day's daily <em>and</em> that week's weekly rather
    /// than being double-charged.
    /// </remarks>
    ...
    /// <exception cref="ForeignEbsSnapshotProtectedException">Any candidate is <see cref="BackupOwnership.Foreign"/>.</exception>
        ...
        foreach (var artifact in artifacts)
        {
            if (artifact.Ownership != BackupOwnership.Servyx)
            {
                throw new ForeignEbsSnapshotProtectedException(
                    ...
                    + "retention policy. Deleting an EBS snapshot cannot be undone and may be removing the only copy "
                    + "of somebody's saves.",
                    artifact.Location);
            }
        }
