...
// `metadata` and first `deployments` entry's `detect`/`image` blocks — see PalworldDefinitionLoader for
// why this is not a full schema-validated parse). A missing or malformed file degrades to the hardcoded
// AdoptionCriteria.PalworldDefault rather than failing startup.
var definition = PalworldDefinitionLoader.TryLoad(AppContext.BaseDirectory);
