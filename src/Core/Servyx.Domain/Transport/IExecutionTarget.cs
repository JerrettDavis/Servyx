...
public interface IExecutionTarget : IAsyncDisposable
{
    ...
    /// <summary>
    /// Writes a file atomically: content is written to a temporary sibling file and then renamed into
    /// place. Returns a receipt including the SHA-256 of the pre-image (or null if the file did not
    /// previously exist). If <paramref name="options"/> specifies an <c>ExpectedPreImageHash</c> that
    /// does not match the file's current content, the write is refused and <see cref="TargetDriftException"/>
